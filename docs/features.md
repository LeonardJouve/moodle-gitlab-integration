...
A webhook is also created in each group repository to monitor submission merge request events.

The webhook listens for:
- **Close** actions on submission merge requests.

When a merge request is closed, GitLab notifies Moodle of the event. Moodle then identifies the corresponding group submission and sends a notification to the group members informing them that their assignment has been graded.

This mechanism allows students to receive automatic feedback about the completion of the evaluation process without requiring manual communication from teachers. It also ensures that grading-related events are automatically reflected within Moodle.
