...
From an architectural perspective, *RAW* is designed to be modular and extensible. While initially focused on GitLab, the system is structured to support future integration with other Git repository hosting services. This ensures long-term adaptability and encourages community contributions.
...
Several implementations exist across different programming languages and ecosystems, including:
...
- #underline[GitLab Haskell] @gitlab_haskell is a Haskell library developed as part of the #underline[Canvas GitLab Integration] @canvas_gitLab_integration research project, which focuses on integrating GitLab with the Canvas learning management system.
...
However, Moodle plugins are designed to be self-contained and should avoid introducing unnecessary external dependencies. For this reason, a dedicated GitLab client is implemented as part of this project, while taking inspiration from existing solutions and following their established design principles.
...
GitHub Classroom heavily inspired *RAW* automated workflows, and its main features are available on it.
...
As mentioned previously, one of Moodle's main strengths is its modularity and extensibility. It was designed around a comprehensive plugin architecture, allowing developers to extend its functionalities through a wide range of plugin types. The appropriate plugin type should be selected based on the specific requirements of the desired functionality.

Some of the available Moodle plugin types include:

- *Activity module* – Provide learning activities within courses (e.g., Forum, Quiz, Assignment).
...
Plugins must be placed in a directory corresponding to their plugin type. The Moodle Core engine automatically detects and registers these plugins during installation or upgrade. Once registered, their code is loaded and executed whenever the associated functionality is required.
...
The plugin defines two custom fields to associate Moodle and GitLab:
...
*RAW* communicates with the Moodle Core engine to retrieve course and user data, create activity instances, and manage Moodle-side information. A dedicated GitLab Client component handles all communication with GitLab.
...
This features section lists the initial requirements identified for the integration. These requirements define the expected behavior and scope of the solution, including the management of GitLab resources and the interaction between Moodle and GitLab.

#figure(
  image("images/module.svg", width: 50%),
  caption: [GitLab module on Moodle course page scaffold]
)

== Teachers <features_teacher>

As a teacher, I want to:

_
- define practical work properties such as name, description, instructions, due date, group size, reviewers
- create an activity module in Moodle so that I can manage a practical work linked to GitLab repositories.
...
#figure(
  image("images/gitlab_resources.svg", width: 85%),
...
#figure(
  image("images/poc_module.png", width: 70%),
  caption: [POC *GitLab* module view for an example SQL Injections assignment]
)
...
An Infrastructure-As-Code approach was adopted, to provide a reproducible and automated infrastructure using #underline[Terraform] @terraform. Terraform uses a declarative configuration model, allowing the desired state of the infrastructure to be described rather than specifying the sequence of operations required to achieve it. It was used to define and provision the required cloud infrastructure, including creating and configuring an AWS EC2 instance with an Elastic IP address.

Once the infrastructure was provisioned, Configuration-as-Code principles were applied using #underline[Ansible] @ansible. Ansible playbooks were created to automate the installation and configuration of the docker containers used.

A simplified Moodle development docker environment is available through my custom #underline[fork] @moodle_docker_fork of the #underline[moodle-docker] @moodle_docker project. The fork adds a simplified configuration and a dedicated cron service to automate the execution of Moodle's built-in tasks system.

The deployed environment consists of a Moodle instance protected by an identity provider. #underline[Authentik] @authentik was selected because it provides an open-source authentication solution that supports modern authentication protocols with minimal configuration, reducing complexity.
...
Infrastructure-As-Code combined with Configuration-As-Code ensures the deployment process is repeatable, automated and versioned. This results in a ready-to-use environment for development and testing.

#figure(
  image("images/iac.svg", width: 57%),
  caption: [Reproducible architecture for RAW development environment]
)

== Target system design

The target solution was designed around a modular architecture separating Moodle-specific logic, GitLab resource management, and API communication. It follows the Moodle Activity module plugin structure and naming conventions described in the official documentation.
...
This phase covered the development of Moodle plugins and the combination of their components as defined in the target solution design. It included implementing the GitLab client, Moodle user interfaces, automated workflows, and required features. Unit tests were developed alongside the implementation to validate individual components and ensure the correctness of the functionalities. Additional manual testing was performed to identify issues, verify integrations, and improve the solution's overall reliability.
...
This chapter presents the main stages of the development and describes the progression of the work throughout the project.
...
The client uses a token based authentication for each request and provides access to dedicated components responsible for different GitLab resource types.
...
#figure(
  ...
  caption: [*RAW* activity module configuration for an example Kubernetes assignment]
)

== Custom field

As described in the paragraph above @custom_field, 2 custom fields were implemented to store additional information required by the integration within the appropriate Moodle contexts:
...
GitLab repository permissions are configured according to user roles:
- Moodle students receive access to their group repository with the *developer* access level.
- Moodle teachers and reviewers receive the* maintainer* access level.

== Database design

For the plugin, 3 new tables are created in the Moodle database to store the relationships between Moodle activities and GitLab resources.

The following diagram describes the data model  of the new tables:
...
#figure(
  ...
  caption: [Teacher dashboard template section for an example Kubernetes assignment],
)
...
#grid(
  ...
  align(center + horizon)[
    #figure(
      ...
      caption: [Teacher dashboard student groups section for an example Kubernetes assignment],
    )
  ],
  align(center + horizon)[
    #figure(
...
#figure(
  ...
  caption: [Student group selection interface for an example Kubernetes assignment],
)
...
#figure(
  ...
  caption: [Student group interface for an example Kubernetes assignment],
)
...
#figure(
  ...
  caption: [GitLab created resources for an example Kubernetes assignment]
)
...
The plugin integrates with Moodle notifications and calendars to provide deadline reminders and keep users informed about relevant events.
...
#figure(
  ...
  caption: [Example of a Moodle *RAW* assignment due calendar event]
)

Notifications are generated for different events:
...
#figure(
  ...
  caption: [Example of a Moodle *RAW* submission reminder notification]
)
...
To ensure the authenticity and integrity of webhook requests, GitLab signs each payload using a module-specific secret generated by Moodle. This secret is stored encrypted in the Moodle database using the same encryption mechanism as for the GitLab tokens.

It is recommended to have the webhook endpoint publicly accessible without any authentication layer on top, as GitLab requests are signed. This remains secure as *RAW* verifies each incoming webhook request before processing the event.

If authentication cannot be publicly accessible, additional authentication information can be configured manually via custom headers in the GitLab webhook settings.

Two webhook types are used: template repository webhooks and group repository webhooks.

=== Template repository webhook

The webhook hooks push events on the `main` branch. When one event occurs, GitLab notifies Moodle that the template repository has been updated. Moodle propagates the changes through merge requests to all group repositories, allowing to incorporate the latest updates. It also hooks issue events to keep the instructions issues synchronized between the repositories. This mechanism allows teachers and reviewers to modify the practical work even after student repositories have been created.

#figure(
  image("images/gitlab_webhook.png"),
  caption: [Example GitLab template repository webhook]
)

=== Group repository webhook

The webhook monitors submission merge request events for each group repository. When a merge request is closed, GitLab notifies Moodle, which identifies the corresponding group submission and updates the submission status. It allows grading-related events to be automatically reflected in Moodle and informs students when their work has been evaluated.

== Protected files

During a practical work, some files should not be modified by students. Thus, a GitLab CI verification job is automatically created in every repository to detect modifications to protected files.

The teachers define which files should not be modified into `.gitlab/protected-files`.

During merge request events, if a modified file matches one of the protected patterns, the pipeline fails and reports an error. This allow students to have an immediate feedback when they modify a protected file and helps reviewers identify changes that may require additional attention during the evaluation process.
...
This chapter presents the final solution delivered by *RAW*. It summarizes the main components of the developed Moodle activity module, the features available for teachers and students, and the automated integration flows.

== Overview

The developed solution provides a Moodle activity module that integrates GitLab into the management of practical assignments. The *RAW* activity module acts as the central entry point for both teachers and students. All information related to an assignment—including the project description, starter code, repositories, submissions, reviews, and the reference solution—is accessible directly from Moodle.
...
The integration plugins maintain synchronization between Moodle and GitLab through bidirectional communication. Actions performed on either platform, such as repository submissions or evaluations, are propagated automatically, ensuring that both systems remain consistent throughout the assignment lifecycle.
...
== Assessment of the Initial Project Specifications

The initial project specifications defined in @spec were successfully fulfilled, with the following additions and limitations identified.

Support for custom GitLab host configuration was added to enhance the solution's flexibility. This is particularly relevant in educational environments, where self-hosted GitLab instances are commonly used to maintain control over infrastructure and user data.

The current implementation supports GitLab exclusively. Although the architecture was designed to be modular and extensible, support for alternative Git hosting platforms, such as GitHub, has not been implemented.

Furthermore, the solution has not been evaluated with a representative group of users. Due to time constraints and the summer holiday period, a comprehensive user validation involving teachers and students could not be conducted.

== Teacher Features

All teacher initial requirements defined in @features_teacher have been implemented, with one exception. The feature allowing teachers to view individual GitLab code insights and statistics was not implemented. With the growing adoption of collaborative development practices like #underline[JetBrains Code With Me] @code_with_me and AI-assisted programming tools like #underline[GitHub Copilot] @copilot, such metrics are becoming less representative of each student's actual contribution and were therefore considered not required features.

Support for protecting specific files via a CI job was introduced as an extension of the initial requirements, inspired by a key feature of GitHub Classroom.

== Student Features

All initial student requirements defined in @features_students have been implemented, with one exception. The plugin does not provide a dedicated interface for viewing Moodle grades. Grading practices differ considerably between institutions and countries, making the implementation of a generic grading solution outside the scope of this project. Instead, teachers and reviewers remain free to define their own grading format through submission merge request comments. Integration with Moodle's existing grading system could be considered as future work.
#pagebreak()
...
Before this project, practical assignment management was requiring manual coordination between Version Control System and Learning Management System. While existing solutions provided partial automation or integration capabilities, they generally missed LMS integration or relied on proprietary platforms.

The project improves the existing workflow by introducing an open-source, Moodle-native integration that connects Moodle and GitLab into a unified assignment management process.

== Limitations

*RAW* still has a few limitations due to restrictions in GitLab's permission model. The main restrictions include:
...
Several areas could be improved in future iterations of the project. 

Additional work is required to strengthen error handling and improve user feedback. Providing clearer error messages and guidance would help users better understand the cause of failures and facilitate troubleshooting.
...
A CI pipeline should be added to the plugin's code repository to automate test execution and prevent invalid code from being merged. This could be achieved by using the #underline[moodle-plugin-ci] @moodle_plugin_ci GitHub action provided by Moodle.
